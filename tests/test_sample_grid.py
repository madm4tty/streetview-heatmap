import pytest
from app.processing import sample_coords, get_adaptive_sample_count
...
def test_sample_coords_all():
    ...
    assert sample_coords(coords, 5) == coords
    assert sample_coords(coords, 3) == coords
...
def test_sample_coords_zero_or_negative():
    ...
    assert sample_coords(coords, 0) == coords
    assert sample_coords(coords, -1) == coords


def test_adaptive_sample_count_motorway():
    """Motorways get more samples."""
    base = 5
    count = get_adaptive_sample_count("motorway", base, 100)
...
def test_adaptive_sample_count_footway():
    ...
    base = 5
    count = get_adaptive_sample_count("footway", base, 100)
...
def test_adaptive_sample_count_unknown():
    ...
    base = 5
    count = get_adaptive_sample_count("unknown_type", base, 100)
...
def test_adaptive_sample_count_respects_coord_limit():
    ...
    base = 10
    count = get_adaptive_sample_count("motorway", base, 5)  # Only 5 coords
    assert count == 5
